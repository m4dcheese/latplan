        ...
        for row in range(size):
            for col in range(size):
                ...
                if random_distribution:
                    top_offset = int(rng.uniform(low=-field_padding, high=((size - 1) * field_resolution) + field_padding))
                    left_offset = int(rng.uniform(low=-field_padding, high=((size - 1) * field_resolution) + field_padding))
                else:
